...
source "proxmox-iso" "pfsense-cloud" {
  ...
  vm_name              = "pfsense-cloud-2.7.2-template"

  template_description = "pfSense Cloud 2.7.2 Template — Site Cloud (vmbr3 DMZ + vmbr4 LAN + CLOUD WAN)"
  ...
  # Bridges utilisés uniquement pour faire démarrer l'installeur pfSense.
  # Terraform réassigne les vrais bridges (CLOUD, vmbr4, vmbr3) au moment du clone.
  # Le bridge CLOUD est OVS/SDN → QEMU ne peut pas s'y connecter directement.

  # vtnet0 = WAN → vmbr5 (WAN public, bridge Linux standard garanti existant)
  network_adapters {
  ...
  # vtnet1 = LAN → vmbr4 (bridge Linux standard garanti existant)
  network_adapters {
  ...
  # vtnet2 = DMZ → vmbr3 (bridge Linux standard)
  # Si vmbr3 n'existe pas encore, remplacer temporairement par vmbr0
  network_adapters {
...
build {
  ...
  sources = ["source.proxmox-iso.pfsense-cloud"]
